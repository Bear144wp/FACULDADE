'''Utilize uma lista para resolver o problema a seguir. Uma empresa paga seus vendedores com base em comissões.
O vendedor recebe $200 por semana mais 9 por cento de suas vendas brutas daquela semana. Por exemplo, um
vendedor que teve vendas brutas de $3000 em uma semana recebe $200 mais 9 por cento de $3000, ou seja, um
total de $470. Escreva um programa (usando um array de contadores) que determine quantos vendedores
receberam salários nos seguintes intervalos de valores:
a. $200 - $299
b. $300 - $399
c. $400 - $499
d. $500 - $599
e. $600 - $699
f. $700 - $799
g. $800 - $899
h. $900 - $999
i. $1000 em diante
Desafio: Crie uma fórmula para chegar na posição da lista a partir do salário, sem fazer vários ifs aninhados.'''

opcoes = ["Windows Server", "Unix", "Linux", "Netware", "Mac OS", "Outro"]
votos = [0] * len(opcoes)

print("As possíveis respostas são:")
for i, opcao in enumerate(opcoes, 1):
    print(f"{i}- {opcao}")

while True:
    try:
        voto = int(input("Digite o número da opção para votar (ou 0 para encerrar): "))
        if voto == 0:
            break
        elif 1 <= voto <= len(opcoes):
            votos[voto - 1] += 1
            print(f"Voto para {opcoes[voto - 1]} computado.")
        else:
            print("Opção inválida. Por favor, digite um número entre 1 e", len(opcoes))
    except ValueError:
        print("Por favor, digite um número inteiro.")

print("\nResultado da votação:")
for i, opcao in enumerate(opcoes):
    print(f"{opcao}: {votos[i]} votos")
